...
# TODO
#   Arreglar start() / boot() y los CAT/SED de los files de LAN4
#
#   Arerglar wg-autoconf show/status
#       Debería mostrar TODAS las interfaces reales, en este caso, USANDO wc show para interceptar todas las que hay
#       Esto arregla lñas residuales
#
#   Pulir menu, help, etc.
...
remove_tagged_block() {
    ...
    # sed with range: /start id X/,/end id X/!p = print all EXCEPT lines between tags with same id
...
backup_config_file() {
    ...
    # 1. Only backup if doesn't exist yet
...
show_usage() {
    ...
    echo ""
    echo "==[ WireGuard Auto-Config for OpenWrt $VERSION ] =========="
    ...
    echo "  setup <conf_name>              - Setup from .conf"
    echo "  setup --advanced               - Interactive setup"
    echo "  setup --advanced [conf]        - Advanced with defaults"
    echo " "
    ...
    echo "  remove <wg_iface>              - Delete WG interface"
    echo " "
    echo "  routes show                    - Show all routes"
    echo "  routes show <wg_iface>         - Show routes for WG interface"
    echo "  routes set <wg_iface> <lan>    - Set routes: WG → LAN"
    echo "  routes unset <wg_iface> <lan>  - Unset routes: WG → LAN"
    echo " "
    echo "  backups show                   - List available backups"
    echo "  backups restore [name]         - Restore backup files"
    echo " "
    echo "  cleanup                        - Clean WG configs (interactive)"
    echo "  cleanup -y                     - Auto cleanup"
    echo "  cleanup emergency              - Manual interface selection"
    echo ""
    echo "Examples:"
    echo "  wg-autoconf setup test"
    echo "  wg-autoconf setup --advanced"
    echo "  wg-autoconf up wg0"
    echo "  wg-autoconf status wg0"
    echo "  wg-autoconf routes set wg5 lan4"
    echo "  wg-autoconf routes show wg5"
    exit 1
}
...
#####################################################################
# WG-AUTOCONF setup NETWORK config, HEREDOC with TAGS (with ID)
...
setup_network_config() {
    ...
    # 2. Check if tag already exists (by ID)
    ...
    # 3. PROCESS ALLOWED IPS (split by comma)
...
EOF
    ...
    # 5. ADD EACH ALLOWED IP AS SEPARATE LIST ITEM
...
#####################################################################
# WG-AUTOCONF setup DHCP config, HEREDOC with TAGS (with ID)
setup_dhcp_config() {
...
#####################################################################
# WG-AUTOCONF setup Firewall config, HEREDOC with TAGS (with ID)
...
setup_wireguard() {
    ...
    # 3. SETTING UP wg INTERFACE (default wg0)
    iface_name="wg0"
    ...
    # 3.2 BACKUP CONFIG FILES (once, first time only)
    backup_config_file "$NETWORK_CONF"
    backup_config_file "$DHCP_CONF"
    backup_config_file "$FIREWALL_CONF"

    # 4. ALLOWED_IPS Handler
    if [ -n "$allowed_ips_raw" ]; then
        allowed_ips=$(process_allowed_ips "$allowed_ips_raw" | head -1)
    else
        allowed_ips="0.0.0.0/0"
    fi

    # 5. SETUP CONFIGS
    setup_network_config "$iface_name" "$private_key" "$address" "$public_key" "$endpoint_host" "$endpoint_port" "$allowed_ips" "1.1.1.1 1.0.0.1"
    ...
    echo "DNS:                 1.1.1.1 1.0.0.1 (Hardcoded values)"
    ...
    echo "4. TEST DNS:          traceroute -i $iface_name google.com"
    echo "5. ROUTE LANs         wg-autoconf routes set $iface_name <lan_iface>"
    echo ""
}


#####################################################################
# WG-AUTOCONF setup ADVANCED MODE (MANUAL CONFIG)
setup_wireguard_advanced() {

    local conf_from_file="$1"
    local conf_file=""

    # If .conf file provided, load defaults from it
    ...
    case "$save_conf" in
        y|Y|yes|YES|Yes)
            ...
            save_wireguard_conf "$conf_filename" "$private_key" "$public_key" "$address" "$endpoint_host" "$endpoint_port" "$allowed_ips"
            ;;
        *)
    ...
    if uci get "network.$iface_name" >/dev/null 2>&1; then
        ...
        read -p "Overwrite interface config? (y/N): " response
    ...
    setup_network_config "$iface_name" "$private_key" "$address" "$public_key" "$endpoint_host" "$endpoint_port" "$allowed_ips" "$dns"
    setup_dhcp_config "$iface_name"
    setup_firewall_config "$iface_name" "$endpoint_port"
    ...
    # 13. RESULTS
    echo ""
    success "Advanced setup completed!"
    echo ""
    echo "**** WireGuard CONFIG (ADVANCED) ****"
    echo "Interface:           $iface_name"
    echo "Local IP:            $address"
    echo "Endpoint:            $endpoint_host:$endpoint_port"
    echo "DNS:                 $dns"
...
activate_interface() {
    ...
    # 1. VERIFY iface provided
    [ -z "$iface" ] && error "Interface name required"

    # 2. VERIFY iface EXISTANCE
...
remove_wireguard() {
    ...
    # 1. EXTRACT TAG IDs FROM CONFIG FILES
    local network_tag_id=$(grep "# wg-autoconf network start id" "$NETWORK_CONF" 2>/dev/null | grep "$iface" -B 5 | grep "# wg-autoconf network start id" | sed 's/.*id //g' | head -1)
    local dhcp_tag_id=$(grep "# wg-autoconf dhcp start id" "$DHCP_CONF" 2>/dev/null | grep "$iface" -B 5 | grep "# wg-autoconf dhcp start id" | sed 's/.*id //g' | head -1)
    local firewall_tag_id=$(grep "# wg-autoconf firewall start id" "$FIREWALL_CONF" 2>/dev/null | grep "$iface" -B 5 | grep "# wg-autoconf firewall start id" | sed 's/.*id //g' | head -1)

    # 2. REMOVE TAGGED BLOCKS BY ID
    [ -n "$network_tag_id" ] && remove_tagged_block "$NETWORK_CONF" "network" "$network_tag_id"
    [ -n "$dhcp_tag_id" ] && remove_tagged_block "$DHCP_CONF" "dhcp" "$dhcp_tag_id"
    [ -n "$firewall_tag_id" ] && remove_tagged_block "$FIREWALL_CONF" "firewall" "$firewall_tag_id"
...
show_status() {
    ...
    for iface in $interfaces; do
        ...
        # If filter specified, skip others
...
#####################################################################
# WG-AUTOCONF cleanup - STANDARD MODE
cleanup_all() {
    ...
    log "Starting COMPREHENSIVE cleanup of WireGuard configuration ..."
    echo ""

    interfaces=$(get_all_wg_interfaces)

    if [ -z "$interfaces" ]; then
        log "No WireGuard interfaces found in UCI"
        echo ""
        log "Cleaning up residual configs, routes, and backups..."
        cleanup_wireguard_routes
        cleanup_config_files "$force_yes"    # ← PASAR FLAG
        success "Cleanup completed!"
        return 0
    fi

    echo "Found WG interfaces in UCI:"
    for iface in $interfaces; do
        echo "  - $iface"
    done
    echo ""
    ...
    if [ $force_yes -eq 1 ]; then
        ...
        log "Automatic cleanup (-y flag detected)"
        echo ""
        for iface in $interfaces; do
            remove_wireguard "$iface" silent 2>/dev/null || true
        done
        cleanup_wireguard_routes
        cleanup_config_files "$force_yes"    # ← PASAR FLAG
        success "All WireGuard configurations successfully removed!"
        echo ""
        return 0
    fi

    read -p "Remove ALL WireGuard configurations? (y/N): " response
...
#####################################################################
# WG-AUTOCONF cleanup - EMERGENCY MODE
cleanup_emergency() {

    log "EMERGENCY cleanup mode - Manual interface selection"
    echo ""
    warning "This will show ALL network interfaces. Select which to remove."
    echo ""

    # 1. GET ALL INTERFACES
    all_ifaces=$(ip link show | grep "^[0-9]" | awk -F': ' '{print $2}' | cut -d: -f1)

    [ -z "$all_ifaces" ] && error "No interfaces found!"
    ...
    # 2. ASK FOR EACH INTERFACE ONE BY ONE
    for iface in $all_ifaces; do

        iface_status=$(ip link show "$iface" 2>/dev/null | grep "state" | awk '{print $9}')
        [ -z "$iface_status" ] && iface_status="UNKNOWN"

        printf "${BLUE}$iface${NC} [$iface_status]: Remove? (y/N): "
        read -r response

        case "$response" in
            y|Y|yes|YES|Yes)
                to_remove="$to_remove $iface"
                success "Marked for removal: $iface"
                ;;
            *)
                log "Skipped: $iface"
                ;;
    ...
    if [ -z "$to_remove" ]; then
        warning "No interfaces selected for removal"
        return 0
    fi

    echo "Interfaces to be removed:"
    for iface in $to_remove; do
        echo "  - $iface"
    done
    echo ""

    # 3. FINAL CONFIRMATION
    read -p "CONFIRM removal of selected interfaces? (y/N): " final_confirm
    ...
    for iface in $to_remove; do
        ...
        success "Removed: $iface"
    done

    echo ""

    # 5. CLEANUP ROUTES (forced)
    log "Cleaning up routing tables and rules..."
    cleanup_wireguard_routes

    # 6. CLEANUP CONFIG FILES (forced restore + cleanup)
    log "Cleaning up configuration files..."

    # Remove ALL network, dhcp, firewall tags
    ...
    # 7. RESTORE BACKUPS (forced, no ask)
...
cleanup_wireguard_routes() {
    ...
    if [ -f /etc/iproute2/rt_tables ]; then
        ...
        grep -v "_vpn" /etc/iproute2/rt_tables > /tmp/rt_tables.tmp 2>/dev/null
    ...
    # 3. FLUSH ALL IP RULES RELATED TO wg
    ip rule show | grep "wg\|vpn" | while read -r line; do
...
cleanup_config_files() {
    ...
    # 3. RESTORE FROM BACKUPS (force en -y, else ask)
    echo ""

    if [ $force_yes -eq 1 ]; then
        # AUTO restore sin preguntar
...
unset_lan_routes() {
    ...
    success "$lan_iface routes (via $wg_iface) succesfully removed!"
}
...
#####################################################################
# MAIN CMD - PARSE ARGUMENTS FLEXIBLY
